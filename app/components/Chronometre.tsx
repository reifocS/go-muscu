...
export default function Chrono() {
  ...
  const [customTimers, setCustomTimers] = useLocalStorage<number[]>(
    "gm-custom-timers",
    []
  );
  ...
  const setTimeAndStore = (time: number) => {
    setTime({
      timer: time,
      finished: false,
      endTime: dayjs().add(time, "seconds"),
    });
    //setLastTime(time);
  };
  ...
  return (
    <div>
      <button
        className="fixed bottom-3 right-3 flex h-[60px] w-[60px] flex-col items-center justify-center rounded-full bg-blue-500 p-4 font-bold text-white transition-colors duration-150"
        onClick={open}
      >
      ...
      >
        <div className="w-full flex-col items-center justify-center text-right">
          ...
          <div className="flex w-full flex-col items-center">
            ...
            <div className="flex gap-1">
              <input
                ...
                className={"w-[70px] text-black"}
                id={"custom-timer"}
                name={"custom-timer"}
                type={"number"}
              />
              <button
                className="bg-blue-700 p-3 hover:bg-blue-800"
                onClick={() => {
                  if (customTimerValue <= 0) return;
                  setCustomTimers((prevState) => [
                    ...prevState,
                    customTimerValue,
                  ]);
              ...
              <button
                className="bg-red-500 p-3 text-sm hover:bg-red-800"
                onClick={() => {
                  setCustomTimers([]);
                }}
              >
          ...
          <button
            className="
              mt-2 flex w-full
              items-center justify-center rounded bg-blue-700 py-3 px-4 font-bold
